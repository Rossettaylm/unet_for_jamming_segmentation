...
B = 50e6;  %信号带宽
taup = 20e-6; %信号脉宽
N = taup * fs; % 一个脉冲宽度的采样点数
samp_num = 6000;%距离窗点

k = - B / taup;
t = linspace(taup/2, 3*taup/2, N);
% sub_pulse_num = 5;

% for sub_pulse_id = 1:sub_pulse_num
% 
% end
...
%     Aj=10^(JNR/20);%干扰回波幅度
    ...
    sp(1+range_tar:N+range_tar)=sp(1+range_tar:N+range_tar)+As*lfm;  %噪声+目标回波 目标在距离窗内200点处


  %% 采样参数设置
    period= 20e-6 / randi([2, 6]);    %采样脉冲周期 
    duty=50;  %占空比
